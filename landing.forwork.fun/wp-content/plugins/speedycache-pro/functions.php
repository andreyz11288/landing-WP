...
function speedycache_pro_load_license($parent = 0){
	...
	// Load license
	$speedycache->license = get_option($license_field, []);
	...
	if(!empty($speedycache->license) && !empty($speedycache->license['license']) && (time() - @$speedycache->license['last_update']) >= 86400){

		$resp = wp_remote_get($license_api_url.'license.php?license='.$speedycache->license['license'].'&url='.rawurlencode(site_url()));
		...
		if(is_array($resp)){
			...
			if(!empty($tosave['license'])){
				...
				update_option($license_field, $tosave, false);
			}
		}
	}
	...
	if(empty($speedycache->license) || empty($speedycache->license['active'])){
		...
		if(function_exists('softaculous_pro_load_license')){
			...
			if(!empty($softaculous_license['license']) && 
				(!empty($softaculous_license['active']) || empty($speedycache->license['license']))
			){
				$speedycache->license = $softaculous_license;
			}
		}elseif(empty($parent)){
			$speedycache->license = get_option('softaculous_pro_license', []);
			
			if(!empty($speedycache->license)){
				speedycache_pro_load_license(1);
			}
		}
	}
